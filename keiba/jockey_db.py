...
def build():
    ...
    acc_t = defaultdict(lambda: [0, 0, 0])  # trainer_id -> 同
    n_out = 0
    for date, rid, d in files:
        ...
        for h in d["horses"]:
            ...
            if tid:
                r, t3, w = acc_t[tid]
                tfeat[str(h["num"])] = dict(
                    tid=tid, t_n=r,
                    t_top3=(t3 + g_top3*PRIOR_N)/(r + PRIOR_N))
        json.dump(dict(race_id=rid, date=date, jockey=feat, trainer=tfeat),
                  open(f"hist_feat/{rid}.json", "w", encoding="utf-8"),
                  ensure_ascii=False)
        n_out += 1
        # レース確定後に累積へ反映(as-of保証)
        for h in d["horses"]:
            if not h.get("rank", "").isdigit():
                continue
            for key, ac in (("jockey_id", acc), ("trainer_id", acc_t)):
                x = h.get(key)
                if not x:
                    continue
                ac[x][0] += 1
                ac[x][1] += (1 if int(h["rank"]) <= 3 else 0)
                ac[x][2] += (1 if int(h["rank"]) == 1 else 0)
    # 予測時用: 全期間集計
    out = {jid: dict(n=r, top3=(t3 + g_top3*PRIOR_N)/(r + PRIOR_N),
                     win=(w + g_win*PRIOR_N)/(r + PRIOR_N))
           for jid, (r, t3, w) in acc.items()}
    out_t = {tid: dict(n=r, top3=(t3 + g_top3*PRIOR_N)/(r + PRIOR_N))
             for tid, (r, t3, w) in acc_t.items()}
    json.dump(dict(global_top3=g_top3, global_win=g_win, jockeys=out, trainers=out_t),
