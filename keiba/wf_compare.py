...
def train_fold(train, spec):
    ...
    Xt, yt, gt = matrix(train[:-n_val], v4)
    Xv, yv, gv = matrix(train[-n_val:], v4)
    if spec.get("binary"):
...
def predict(model, r, v4):
    M = np.array([V2.build_row(r, n, v4) for n in r["ns"]], dtype=np.float32)
    s = model.predict(M)
