...
寸法の根拠：
  ...
  ・ゴミ置き場 2,000×1,000 … よく指定される大きさ（当日の指定が優先）
  ・敷地内通路 1.5m（階数3以下かつ延べ200㎡未満は90cm）… 令128条
出力：
...
def sizes():
    ...
    s.text(x + 190, y + 96, 'よく指定される大きさ。', size=11.5,
           anchor='start', fill='#444')
    s.text(x + 190, y + 116, '当日の指定が優先。', size=11.5,
           anchor='start', fill='#444')
...
def haichi():
    ...
    ROWS = (('敷地境界線と建築物との距離',
            ...
            ('玄関の土間の高さ', '「GL+400」など、地盤面からの高さ（ポーチも）'),
            ('玄関ホールの床高', '「GL+550」'),
            ('方位（N）', '敷地図と同じ向きに'))
