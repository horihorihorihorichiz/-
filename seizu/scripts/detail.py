...
高さの基準（GL＝地面）：
  基礎の底         GL -300 （根入れ 300mm。べた基礎の法定最低は120mm＋凍結深度）
  基礎の底盤 厚150  GL -300 ～ -150
  基礎の立上り 厚150 GL -150 ～ +371 （地上部分 371mm。300mm以上が必要）
  基礎パッキン t20  GL +371 ～ +391
  土台 120×120     GL +391 ～ +511
  構造用合板 t24    GL +511 ～ +535
  フローリング t15  GL +535 ～ +550  ＝ 1FL（GL+550）
  1階の階高 3,100   → 2FL ＝ GL +3,650
  胴差 120×300     GL +3,311 ～ +3,611
...
def draw():
    s = Svg(W, H)
    s.add(PATTERNS)
    s.text(W / 2.0, 34, '部分詳細図　Ａ—Ａ（外壁 断面） 1/20', size=21, weight='700')
    s.text(W / 2.0, 57,
    ...
    # ================= 上のパネル：2階床まわり =================
    # 2階の柱＋断熱
    band(s, -60, 60, TOP_HI, 3611, 'url(#pIns)', stroke='#8a6a35',
         sw=1.4)
    # 胴差
    ...
    # ================= 下のパネル：基礎まわり =================
    # 地面
    band(s, 103, xout, 0, BOT_LO, 'url(#pGround)', stroke='none')
    ...
    band(s, -75, 75, 371, -150, 'url(#pConc)', stroke='#666', sw=1.2)
    # 床下
    s.text(X(-260), Y(80), '床下', size=10.5, fill='#999')
    # 基礎パッキン
    band(s, -60, 60, 391, 371, '#d9d9d9', stroke='#555', sw=1.0)
    # 土台
    ...
    band(s, -60, 60, BOT_HI, 511, 'none', stroke='#8a6a35', sw=1.4)
    # 外壁（下）
    band(s, 60, 69, BOT_HI, 511, '#f0e2c8')
    s.line(X(69), Y(BOT_HI), X(69), Y(430), stroke='#2f7fd0',
           stroke_width=1.6)
    band(s, 69, 87, BOT_HI, 430, '#f7f7f7')
    band(s, 87, 103, BOT_HI, 420, '#cfd8dc')
    band(s, -75, -60, BOT_HI, 550, '#ebebeb')
    # 水切り
    s.poly([(X(69), Y(430)), (X(100), Y(400)), (X(112), Y(372))],
    ...
    bot_notes = [
        ...
        (0, 715, 460, '柱 120×120 ＋ グラスウール16K t=100'),
        (-60, 665, 482, '強化石膏ボード t=15（内壁仕上げ）'),
        (-10, 660, 504, '筋かい 45×90 たすき掛け（壁倍率4.0・破線）'),
        (-200, 542, 526, 'フローリング t=15 ＋ 構造用合板 t=24'),
        (-250, 486, 548, '床断熱 押出法ポリスチレンフォーム t=50'),
        (0, 458, 570, '土台 120×120（防腐防蟻処理）'),
        (0, 396, 592, '基礎パッキン t=20（床下の換気口をかねる）'),
        (-20, 300, 614, 'アンカーボルト M12 @2,000以下（埋込み250以上）'),
        (100, 400, 636, '水切り（雨を外へ落とす）'),
        (0, 100, 662, 'べた基礎 立上り 厚150（地上371mm ＝ 300mm以上）'),
        (-200, -225, 690, 'べた基礎 底盤 厚150（根入れ300mm）'),
