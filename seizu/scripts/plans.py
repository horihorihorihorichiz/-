...
FLOORS = {
    1: dict(
        ...
        rooms=[
            ...
            ('店舗売場',      '26.49', 2, 0, 8, 6, 'shop'),   # 東南の角2×2を便所にしたL字
            ('店舗用便所',    '3.31', 6, 4, 8, 6, 'water'),   # 売場から直接入る（厨房を通らない）
        ...
        doors=[
            ('H', 2, 1.0, 1.0),   # 玄関 → 階段（片引き戸。下足入れは西の壁ぎわ）
            ('V', 2, 2.2, 1.0),   # 階段（南の上りはじめ側）→ 売場（竪穴区画：防火設備）
            ('H', 6, 4.0, 1.0),   # 売場 → 厨房（直接行き来）
            ('V', 6, 4.0, 1.0),   # 売場 → 店舗用便所（客は厨房を通らない）
            ('V', 2, 9.0, 1.0),   # 厨房 → 倉庫（北の端。西の壁に流し台の列を置く）
            ('V', 5, 9.0, 1.0),   # 厨房 → スタッフルーム（北の端）
        ],
...
def fit_doors(d, floors=None):
    ...
    for ori, wall, pos, ln in d.get('doors', []):
        ...
        p = round(p / STEP) * STEP
        p = min(max(p, a), b - w)
        # 建具はすべて引き戸。片引き戸は横の壁に戸1枚ぶんの「引込み」が要るので、
        # 両わきとも足りないときは近いほうの壁の端へ寄せる（壁が2枚ぶんあるとき）
        if (p - a < w - 1e-6 and b - (p + w) < w - 1e-6) and b - a >= 2 * w - 1e-6:
            p = a if (p - a) <= (b - (p + w)) else b - w
        ok = True
