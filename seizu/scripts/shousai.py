...
s.text(W / 2.0, 55, 'かべを切って中身を見せる図。よこ線を5本引いてから、間をうめる',
       size=12, fill='#666')
...
# ============================================================
band(74, 360, 'その1', 'まず「よこ線」を5本引く（ノートの罫線と同じ）')
...
STEPS = [('よこ線を5本', '地面・基礎の上・1階の床・天じょう・2階の床'),
         ('たて線（柱のはば120）', '柱の左と右。まん中の線は通り芯'),
         ('基礎をかく', '地面から上に371、地面の下に300'),
         ('木をかく', '土台・床の板・胴差（1階と2階をつなぐ木）'),
         ('かべの層をかく', '内から外へ 15・120・9・18・16'),
         ('数字と名前', '左に高さ、右に材料の名前')]
...
# ============================================================
band(710, 1010, 'その3', '書きこむのは「高さ」と「材料の名前」だけ')
...
BOX = [(70, '左がわに書く＝高さ', INK,
        ['GL（地面）　±0', '基礎の上の面　GL＋371', '1FL　GL＋550',
