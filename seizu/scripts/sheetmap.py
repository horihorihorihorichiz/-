...
ITEMS = [
    ...
    ('6', 0.62, 0.790, 'タイトル欄', '課題名など'),
    ('7', 0.30, 0.935, '解答のポイント', 'この教材だけの欄。本番にはない'),
]
...
def main():
    ...
    s = Svg(W, H)
    s.text(W / 2.0, 30, 'この教材の標準解答例・1枚目にある、7つの枠', size=20,
           weight='700')
    s.text(W / 2.0, 51,
           '本番はA2横の紙1枚に、これ＋伏図・立面図・部分詳細図・計画の要点等が'
           '全部載る。左から右へ、上から下へ埋めていく',
           size=11.5, fill='#666')
