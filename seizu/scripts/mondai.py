...
Y_KOZO = ('木造３階建てとするに当たり、構造計画上工夫した点',
          ...
          '通し柱、その他は120mm角の管柱としている。梁の最大スパンは'
...
Y_DOUSEN = ('店舗部分と住宅部分の動線計画について工夫した点',
            '店舗の出入口は道路側に幅2,730mmで設け、来店客は店舗売場から'
            '店舗用便所へ至る動線とした。店舗用便所は売場の一部を区画して'
            '設け、厨房を通らずに利用できるようにしている。住宅の玄関は'
            '店舗の出入口から3,640mm離して別に設け、有効910mmの廊下で'
...
SPECS = [
 ...
 dict(tag='B', name='１階に母の寝室',
      ...
      youten=[
        ...
        ('高齢者の避難について配慮した点',
         ...
         'できる。寝室は南面に幅1,820mmの掃出し窓、西面に幅1,365mmの窓を'
         '設けた２方向避難とし、掃出し窓から直接屋外へ出ることもできる。'
         '廊下から玄関までの避難経路は有効910mmの直線で、途中に段差や'
         '狭くなる部分がない。階段室は準耐火構造の床及び壁で囲んだ竪穴'
         '区画としているため、上階からの煙が１階の廊下へ流れ込みにくく、'
         '高齢者でも落ち着いて屋外へ出られる。')],
 ...
 dict(tag='E', name='南東の角地',
      ...
      youten=[
        ('角地であることを活かして工夫した点',
         '店舗の出入口を南東の角に向けて幅2,730mmで設け、南側の通りからも'
         '東側の通りからも入りやすくした。店舗売場は南面に幅3,640mm、'
         '東面に幅2,730mmの開口部を持つため、自然光が奥まで入り、通りから'
         'も店内の様子が見える。住宅の玄関は南面の西端に、店舗の出入口'
         'から3,640mm離して設けた。これにより角地の２面を店舗の顔として'
         '使いながら、来店客と住まい手の出入りが重ならない。'),
        ('容積率及び建蔽率の算定について',
 ...
 dict(tag='F', name='北側道路',
      ...
      youten=[
        ('北側道路に対して、住宅部分の日照をどのように確保したか',
         ...
         '子ども室はいずれも南面に幅1,820mm以上の開口部を設けて日照を'
         '確保している。住宅の玄関は北西に設け、有効910mmの廊下１本で'
...
def kaitou(sp):
    ...
    o.append('<div class="warn">どの解答例も、つぎの<b>4つの部品を'
             'この順につないだだけ</b>です。'
             '<b>①なにをしたか</b> → <b>②数字を入れる</b>'
             '（120mm角・3,640mm・t=24・有効780mmなど）→ '
             '<b>③なぜそうしたか</b> → <b>④どうなるか</b>。'
             '文章を丸暗記せず、この4つの順番と、分野ごとの'
             '「部品」だけを覚えてください。</div>')
