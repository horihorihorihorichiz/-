...
s.text(W / 2.0, 55, '横の位置は平面図から写す。たての位置は床から決める',
       size=12, fill='#666')
...
CAP = [('外わくの長方形', '少し太い線で'),
       ('内がわにもう1つ', '5mmくらい内側。これが枠の見込み'),
       ('まん中にたて線1本', '引違い（2枚）なら1本')]
...
s.text(90, 254, '※ 4枚建てならたて線3本。上に欄間があるなら横線を1本足す。',
       size=11, anchor='start', fill='#555')
s.text(90, 276, '※ 出入口（ドア）も同じ描き方。ちがうのは高さだけ。',
       size=11, anchor='start', fill='#555')

# ============================================================
band(310, 640, 'その2', 'たての位置は「床から」決める')
K = 0.055                       # 1mmあたりのピクセル
...
s.text((a1 + b1) / 2.0, by + 22, '腰窓（こしまど）', size=11.5, weight='700')
...
s.text((a2 + b2) / 2.0, by + 22, '掃き出し窓・出入口', size=11.5,
       weight='700')
s.text((a2 + b2) / 2.0, by + 38, 'FL＋0 〜 FL＋2,000', size=10, fill=RED)
s.text(70, 352, '窓の高さは「その階の床から」測る。'
       'どの階も同じ数字にすると、立面がそろってきれいに見える。',
       size=11, anchor='start', fill='#555')

# ============================================================
band(650, 950, 'その3', '横の位置は平面図から真下に写す')
px0, pw = 130, 340
py0 = 700
# 平面図（南面の壁だけ）
...
NG = [('平面図と位置がちがう',
       '「図面相互の不整合」で大きな減点。必ず線を下ろして写す'),
      ('階ごとに高さがバラバラ',
       '腰窓はどの階も FL＋800 にそろえる。見た目もきれいになる'),
      ('1階の窓が基礎にめり込む',
       '窓は 1FL より下に描かない。1FL＝GL＋550 の線を先に引いておく'),
      ('バルコニーの手すりを忘れる',
       '掃き出し窓の前に格子。高さ1,100以上（3階建ては必要）')]
