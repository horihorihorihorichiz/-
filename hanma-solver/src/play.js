...
function renderHint() {
  ...
  if (reviewEntry) {
    ...
    box.innerHTML = `🔍 <strong>${l.turn}手目の局面</strong>　` +
      ...
      (l.ok ? ' <span class="rv-ok">✓ 問題なし（期待値ほぼ互角）</span>' : ` <span class="rv-bad">${reviewReason(l)}</span>`);
    return;
  }
...
function renderReview() {
  ...
  h += `<div class="rv-head">🔍 打牌レビュー（総合評価）<span class="rv-score">問題なし ${okCnt}/${total}</span></div>`;
  ...
  h += `<div class="rv-note">期待値が明確に落ちた<strong>${misses.length}手</strong>だけ表示（誤差レベルの手は省略）。「局面 →」でその盤面に戻れます。</div>`;
  h += `<div class="rv-table-wrap"><table class="rv-table"><thead><tr>` +
    `<th>打</th><th>あなたの打牌</th><th>期待値ロス</th><th>推奨（1位）</th><th>ひとこと</th><th></th></tr></thead><tbody>`;
  for (const { l, i } of misses) {
    // 期待値ロスの大きさで色分け（順位では絞らない）。向聴が違うと%比較は無効なので向聴差で表示。
    ...
    const lossCell = `<span class="${big ? 'rv-rk-bad' : 'rv-rk-mid'}">${lossTxt}<span class="rv-num"> (${l.rank}位/${l.total})</span></span>`;
  ...
  h += `</tbody></table></div>`;
  h += `<div class="rv-note">※「期待値ロス」＝最善打牌に対する総合期待値（向聴・受け入れ・ドラ・放銃リスク込み）の低下率。誤差(${Math.round(EV_TOLERANCE * 100)}%未満)は省略。</div>`;
  h += `</div>`;
  return h;
}
...
function recordMyDiscard(p, tile) {
  ...
  // 「ミス」判定は順位でなく期待値(value)の差で。向聴が同じで期待値がほぼ互角なら許容（＝上げない）。
  const lossFrac = best.value > 0 ? (best.value - chosen.value) / best.value : 0;
  const ok = chosen.shanten === best.shanten && lossFrac <= EV_TOLERANCE;
  myLog.push({
    ...
    ok, lossFrac, // ok=期待値ほぼ互角（許容）／lossFrac=最善に対する期待値ロス割合
