...
競馬の予想依頼が来たら **必ず先に `keiba/MEMORY.md`（記憶台帳＝現在の体制と確定事実）、`keiba/RULES.md`（ルール・教訓）、`keiba/SOP.md`（作業手順）を読む**。何かを確定・棄却・変更したセッションは終了前に MEMORY.md へ追記する（フォーマットは同ファイル冒頭）。モデルが変わっても、この2つ＋スクリプト群だけで同じ品質で回せる。
